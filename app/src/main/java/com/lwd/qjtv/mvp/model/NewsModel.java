...
public class NewsModel extends BaseModel implements NewsContract.Model {
    ...
    @Override
    public Observable<BBSNewsListBean> getBBSNews(@QueryMap Map<String, String> map) {
        return mRepositoryManager.obtainRetrofitService(UserService.class).getBBSNews(map);
    }

    @Override
    public Observable<BannerBean> getBanner(@QueryMap Map<String, String> map) {
        return mRepositoryManager.obtainRetrofitService(UserService.class).getBanner(map);
    }
}