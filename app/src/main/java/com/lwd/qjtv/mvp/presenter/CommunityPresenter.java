...
import com.lwd.qjtv.mvp.ui.adapter.CommunityAdapter;
...
public class CommunityPresenter extends BasePresenter<CommunityContract.Model, CommunityContract.View> {
    ...
    private List<BBSDataBean.DataBean.ListBean> mList = new ArrayList<>();
    ...
    public void getBBSData(final boolean pullToRefresh) {
        if (mAdapter == null) {
            mAdapter = new CommunityAdapter(mList);
...
//        }, mRootView.getRxPermissions(), mRootView, mErrorHandler);
        ...
        RxUtils.applyListSchedulers(mRootView, mModel.getBBSData(map), pullToRefresh)
                .subscribe(new ErrorHandleSubscriber<BBSDataBean>(mErrorHandler) {
                    @Override
                    public void onNext(@NonNull BBSDataBean list) {
                        ...
                        if (list.getData().getNotification() != null) {
                            Integer integer = Integer.valueOf(list.getData().getNotification());
                            mRootView.setData(integer);
                        } else {
                            mRootView.setData(0);
                        }

                        List<BBSDataBean.DataBean.ListBean> data = list.getData().getList();
