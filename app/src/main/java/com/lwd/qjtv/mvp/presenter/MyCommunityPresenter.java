...
public class MyCommunityPresenter extends BasePresenter<MyCommunityContract.Model, MyCommunityContract.View> {
    ...
    private List<MyCommunityListDataBean.DataBean> mList = new ArrayList<>();
    ...
    @Inject
    public MyCommunityPresenter(MyCommunityContract.Model model, MyCommunityContract.View rootView
    ...
    public void requestCommunityList(final boolean pullToRefresh) {
        if (mAdapter == null) {
            mAdapter = new MyCommunityAdapter(mList);
...
//        }, mRootView.getRxPermissions(), mRootView, mErrorHandler);
        ...
        RxUtils.applyListSchedulers(mRootView, mModel.getMyBBSData(map), pullToRefresh)
                .subscribe(new ErrorHandleSubscriber<MyCommunityListDataBean>(mErrorHandler) {
                    @Override
                    public void onNext(@NonNull MyCommunityListDataBean list) {
                        if (list == null || list.getData() == null) {
                            if (mList == null || mList.size() == 0)
                                mRootView.setData("-1");
                            mRootView.showMessage("当前无更多数据~");
                            if (pullToRefresh) {
                                mRootView.setData(true);
                            }
                            return;
                        }
                        ...
                        mRootView.setData(false);
                        List<MyCommunityListDataBean.DataBean> data = list.getData();
