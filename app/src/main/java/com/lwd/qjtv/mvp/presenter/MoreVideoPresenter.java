...
import com.lwd.qjtv.mvp.ui.adapter.MoreVideoAdapter;
...
@ActivityScope
public class MoreVideoPresenter extends BasePresenter<MoreVideoContract.Model, MoreVideoContract.View> {
    private RxErrorHandler mErrorHandler;
    private AppManager mAppManager;
    private Application mApplication;
    private List<MoreVideoBean.DataBean> mList = new ArrayList<>();
    ...
    @Inject
    public MoreVideoPresenter(MoreVideoContract.Model model, MoreVideoContract.View rootView, RxErrorHandler handler
    ...
    public void requestMoreVideoList(final boolean pullToRefresh, String v_type) {
        if (mAdapter == null) {
            mAdapter = new MoreVideoAdapter(mList);
        ...
        RxUtils.applyListSchedulers(mRootView, mModel.getMoreVideoList(map), pullToRefresh)
                .subscribe(new ErrorHandleSubscriber<MoreVideoBean>(mErrorHandler) {
                    @Override
                    public void onNext(MoreVideoBean moreVideoBean) {
                        List<MoreVideoBean.DataBean> data = moreVideoBean.getData();
                        if (moreVideoBean == null || data == null || !"1".equals(moreVideoBean.getStatus())) {
                            if (mList == null)
                                mRootView.setData(-1);
                            if (!"1".equals(moreVideoBean.getStatus()))
                                mRootView.showMessage(moreVideoBean.getMsg());
                            return;
                        }
