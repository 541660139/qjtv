...
public class CollectionPresenter extends BasePresenter<CollectionContract.Model, CollectionContract.View> {
    ...
    private List<VideoCollectionBean.DataBean> mList = new ArrayList<>();
    ...
    @Inject
    public CollectionPresenter(CollectionContract.Model model, CollectionContract.View rootView, RxErrorHandler handler
    ...
    public void requestCollectionList(final boolean pullToRefresh) {
        if (mAdapter == null) {
            mAdapter = new CollectionAdapter(mList);
...
//        }, mRootView.getRxPermissions(), mRootView, mErrorHandler);
        ...
        RxUtils.applyListSchedulers(mRootView, mModel.getVideoCollection(map), pullToRefresh)
                .subscribe(new ErrorHandleSubscriber<VideoCollectionBean>(mErrorHandler) {
                    @Override
                    public void onNext(VideoCollectionBean list) {
                        ...
                        List<VideoCollectionBean.DataBean> data = list.getData();
    ...
    public void addVideoCollection(String videoId, String videoType) {
    ...
    public void deleteVideoCollection(String fid) {
        Map<String, String> map = new HashMap<>();
        map.put("id", fid);
        map.put("appid", Constant.APPID);
        map.put("op", "batch_cancel_collect");
        map.put("pt", Constant.PT);
        map.put("ver", WEApplication.getApp_ver());
        map.put("imei", WEApplication.getIMEI());
        map.put("uid", Preference.get(Constant.UID, "0"));
        map.put("t", System.currentTimeMillis() + "");
        map.put("sign", Utils.buildSign(map, Constant.key));
        RxUtils.applyNormalSchedulers(mRootView, mModel.deleteVideoCollection(map))
