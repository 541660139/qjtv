...
@ActivityScope
public class AddressManagerPresenter extends BasePresenter<AddressManagerContract.Model, AddressManagerContract.View> {
    ...
    @Inject
    public AddressManagerPresenter(AddressManagerContract.Model model, AddressManagerContract.View rootView, RxErrorHandler handler
    ...
    public void requestAddressManagerList(final boolean pullToRefresh) {
        if (mAdapter == null) {
            mAdapter = new AddressManagerAdapter(mList);
...
//        }, mRootView.getRxPermissions(), mRootView, mErrorHandler);
        ...
        RxUtils.applyListSchedulers(mRootView, mModel.getAddressList(map), pullToRefresh)
                .subscribe(new ErrorHandleSubscriber<AddressBean>(mErrorHandler) {
                    @Override
                    public void onNext(AddressBean addressBean) {
                        List<AddressBean.DataBean> data = addressBean.getData();
                        if (addressBean == null || data == null || !"1".equals(addressBean.getStatus())) {
...
//                            return;
    ...
    public void deleteAddress(String id) {
        ...
        map.put("t", System.currentTimeMillis() + "");
        map.put("sign", Utils.buildSign(map, Constant.key));
        RxUtils.applyNormalSchedulers(mRootView, mModel.deleteAddress(map))
                .subscribe(new ErrorHandleSubscriber<BaseBean>(mErrorHandler) {
                    @Override
                    public void onNext(@NonNull BaseBean baseBean) {
                        mRootView.showMessage(baseBean.getMsg());
                        requestAddressManagerList(true);
                    }
                });
