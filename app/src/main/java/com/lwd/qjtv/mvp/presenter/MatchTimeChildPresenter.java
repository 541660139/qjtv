...
import com.lwd.qjtv.mvp.ui.adapter.MatchTimeChildAdapter;
...
@ActivityScope
public class MatchTimeChildPresenter extends BasePresenter<MatchTimeChildContract.Model, MatchTimeChildContract.View> {
    private RxErrorHandler mErrorHandler;
    private AppManager mAppManager;
    private Application mApplication;
    private List<MatchTimeList.DataBean> mList = new ArrayList<>();
    ...
    @Inject
    public MatchTimeChildPresenter(MatchTimeChildContract.Model model, MatchTimeChildContract.View rootView, RxErrorHandler handler
    ...
    public void requestMatchTimeChildList(final boolean pullToRefresh, String matchId) {
        if (mAdapter == null) {
            mAdapter = new MatchTimeChildAdapter(mList);
...
//        }, mRootView.getRxPermissions(), mRootView, mErrorHandler);
        ...
        RxUtils.applyListSchedulers(mRootView, mModel.getMatchTimeList(map), pullToRefresh)
                .subscribe(new ErrorHandleSubscriber<MatchTimeList>(mErrorHandler) {
                    @Override
                    public void onNext(MatchTimeList list) {
                        List<MatchTimeList.DataBean> data = list.getData();
                        if(list == null)
                            mRootView.setData(-1);
                        if (data == null || !"1".equals(list.getStatus())) {
