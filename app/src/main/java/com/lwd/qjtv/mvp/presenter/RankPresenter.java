...
public class RankPresenter extends BasePresenter<RankContract.Model, RankContract.View> {
    ...
    @Inject
    public RankPresenter(RankContract.Model model, RankContract.View rootView, RxErrorHandler handler
    ...
    public void requestRankList(final boolean pullToRefresh, boolean isGuessRank) {
        if (mAdapter == null) {
    ...
    public void requestNBARankList(final boolean pullToRefresh, boolean isGuessRank) {
        if (mAdapter == null) {
            mAdapter = new RankAdapter(mList);
            mRootView.setAdapter(mAdapter);//设置Adapter
        }
        ...
        map.put("t", System.currentTimeMillis() + "");
        map.put("sign", Utils.buildSign(map, Constant.key));
        RxUtils.applyListSchedulers(mRootView, mModel.guessNBARank(map), pullToRefresh)
                .subscribe(new ErrorHandleSubscriber<GuessRankBean>(mErrorHandler) {
                    @Override
                    public void onNext(@NonNull GuessRankBean guessRankBean) {
                        if (guessRankBean == null)
