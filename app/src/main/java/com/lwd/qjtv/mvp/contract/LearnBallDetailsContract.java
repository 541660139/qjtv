...
public interface LearnBallDetailsContract {
    //对于经常使用的关于UI的方法可以定义到BaseView中,如显示隐藏进度条,和显示文字消息
    interface View extends IView {
        void videoParseFinish(VideoParse videoParse);

        void videoDownloadParseFinish(VideoParse videoParse);
    }

    //Model层定义接口,外部只需关心model返回的数据,无需关心内部细节,及是否使用缓存
    interface Model extends IModel {
//        Observable<LearnBallDetailsBean> getLearnBallDetails(@Query("ac") String ac, @Query("starId") String starId, @Query("page") String page, @QueryMap Map<String, String> map);
