...
//                                JSONArray array = new_pic JSONArray(httpResult);
...
//                        }
                 ...
                 /* 这里如果发现token过期,可以先请求最新的token,然后在拿新的token放入request里去重新请求
                    ...
                    create a new_pic request and modify it accordingly using the new_pic token
                    ...
                    如果不需要返回新的结果,则直接把response参数返回出去 */
                        Log.d("GlobalConfiguration", "onHttpResultResponse: " + response);
                        return response;
                    }
                ...
                /* 用来提供处理所有错误的监听
                   rxjava必要要使用ErrorHandleSubscriber(默认实现Subscriber的onError方法),此监听才生效 */
                    if (e instanceof SocketTimeoutException) {
                        UiUtils.SnackbarText("连接失败，请稍后重试");
                    } else if (e instanceof JSONException) {
                        UiUtils.SnackbarText("解析失败，请稍后重试");
                    } else if (e instanceof ConnectException) {
                        UiUtils.SnackbarText("无法连接，请稍后重试");
                    } else {
                        UiUtils.SnackbarText("网络错误，请稍后重试");
                        Timber.e(e.getMessage(), e.getClass().getName());
                    }
                })
...
//                    retrofitBuilder.addConverterFactory(FastJsonConverterFactory.create());//比如使用fastjson替代gson
                ...
                .okhttpConfiguration((context1, okhttpBuilder) -> {//这里可以自己自定义配置Okhttp的参数
                    okhttpBuilder.writeTimeout(2000, TimeUnit.SECONDS);
                    okhttpBuilder.connectTimeout(2000, TimeUnit.SECONDS);
                    okhttpBuilder.readTimeout(2000, TimeUnit.SECONDS);
