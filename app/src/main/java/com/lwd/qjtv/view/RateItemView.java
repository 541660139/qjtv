...
public class RateItemView extends LinearLayout {
    ...
    public void setData(WatchBallBean.DataBean.LiveListBean.DataListBean data) {
        if (data == null) {
...
*   原有规则
* */
//            if (Preference.getBoolean(Constant.IS_GUESS) && data.getNot_jc() != 1&&Preference.getBoolean(Constant.ON_OFF)) {
...
//            }
            ...
            Log.d("timeStamp", "startTimestamp:------" + startTimestamp);
/**
*   开始时间小于当前时间时屏蔽竞猜
* */
            int i = startTimestamp.compareTo(timestamp);
            if (i < 0) {
                leftTv.setVisibility(GONE);
            }
