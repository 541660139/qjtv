    ...
    private void initIJKPlayer(Message msg) {
        ...
        try {
            if (GSYVideoType.isMediaCodec()) {
                ...
                //                ((IjkMediaPlayer)mediaPlayer).setOption(IjkMediaPlayer.OPT_CATEGORY_FORMAT, "protocol_whitelist", "concat,ffconcat,file,subfile,http,https,tls,rtp,tcp,udp,crypto");
                //                ((IjkMediaPlayer) mediaPlayer).setOption(IjkMediaPlayer.OPT_CATEGORY_PLAYER, "safe", 0);
                //                ((IjkMediaPlayer) mediaPlayer).setOption(IjkMediaPlayer.OPT_CATEGORY_FORMAT, "k_safe", 0);
            }
            //就是这里

            if (((GSYModel) msg.obj).getMapHeadData().containsKey("User-Agent")) {
                ((IjkMediaPlayer) mediaPlayer).setOption(IjkMediaPlayer.OPT_CATEGORY_FORMAT, "user_agent", ((GSYModel) msg.obj).getMapHeadData().get("User-Agent"));
            } else {
//                ((IjkMediaPlayer) mediaPlayer).setOption(IjkMediaPlayer.OPT_CATEGORY_FORMAT, "user_agent", "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/602.1.38 (KHTML, like Gecko) Version/10.0 Mobile/14A300 Safari/602.1");
                ((IjkMediaPlayer) mediaPlayer).setOption(IjkMediaPlayer.OPT_CATEGORY_FORMAT, "user_agent", "Mozilla/5.0 (iPad; CPU OS 10_0 like Mac OS X) AppleWebKit/602.1.38 (KHTML, like Gecko) Version/10.0 Mobile/14A300 Safari/602.1");
            }
            ((IjkMediaPlayer) mediaPlayer).setOption(IjkMediaPlayer.OPT_CATEGORY_FORMAT, "protocol_whitelist", "concat,ffconcat,file,subfile,m3u8,http,https,flv,ts,tls,rtp,tcp,udp,crypto,rtmp");
            ((IjkMediaPlayer) mediaPlayer).setOption(IjkMediaPlayer.OPT_CATEGORY_PLAYER, "probsize", 1024 * 32 + "");
            ((IjkMediaPlayer) mediaPlayer).setOption(IjkMediaPlayer.OPT_CATEGORY_PLAYER, "safe", 0);
            if (((GSYModel) msg.obj).getMapHeadData().containsKey("Referer")) {
                Map refer = new HashMap<String, String>();
                refer.put("Referer", ((GSYModel) msg.obj).getMapHeadData().get("Referer"));
                ((IjkMediaPlayer) mediaPlayer).setDataSource(((GSYModel) msg.obj).getUrl(), refer);
            } else {
