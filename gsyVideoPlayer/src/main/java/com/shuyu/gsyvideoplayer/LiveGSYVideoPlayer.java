...
public abstract class LiveGSYVideoPlayer extends GSYBaseVideoPlayer implements View.OnClickListener, View.OnTouchListener, SeekBar.OnSeekBarChangeListener, TextureView.SurfaceTextureListener {
    ...
    public boolean setUp(List<GSYVideoModel> gsyVideoModels, int tDuration, boolean cacheWithPlay, int position, File cachePath, Map<String, String> mapHeadData, Object... objects) {
        ...
        if (setUp(gsyVideoModels.get(t).getUrl(), cacheWithPlay, cachePath, objects)) {
            this.mMapHeadData.clear();
            this.mDuration = tDuration * 60 * 1000;
            if (mapHeadData != null)
    ...
    public boolean setUp(String url, boolean cacheWithPlay, File cachePath, Object... objects) {
        ...
        mOriginUrl = url;
        if (isCurrentMediaListener() && (System.currentTimeMillis() - CLICK_QUIT_FULLSCREEN_TIME) < FULL_SCREEN_NORMAL_DELAY)
            return false;
        mCurrentState = CURRENT_STATE_NORMAL;
        if (cacheWithPlay && url.startsWith("http") && !url.contains("127.0.0.1") && !url.contains("m3u8")/*&&!url.contains("sohu")&&!url.contains("qiyi")*/
                /*&&!url.startsWith("file")*/) {
            HttpProxyCacheServer proxy = GSYVideoManager.getProxy(getContext().getApplicationContext(), cachePath);
            //此处转换了url，然后再赋值给mUrl。
            //            url = proxy.getProxyUrl("http://vr.tudou.com/v2proxy/v?sid=95001&id=495173636&st=5&pw=");
