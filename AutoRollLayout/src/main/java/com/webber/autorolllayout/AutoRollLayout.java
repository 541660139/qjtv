...
public class AutoRollLayout extends FrameLayout {
    ...
    List<AutoRollLayout.IShowItem> items = new ArrayList<>();
...
//    }
    ...
    private void initImageloader() {
        ...
        options = new DisplayImageOptions.Builder()
                .showStubImage(R.drawable.loding_auto) // 设置图片下载期间显示的图片
                .showImageForEmptyUri(R.drawable.loding_auto) // 设置图片Uri为空或是错误的时候显示的图片
                .showImageOnFail(R.drawable.loding_auto) // 设置图片加载或解码过程中发生错误显示的图片
...
//                 .displayer(new RoundedBitmapDisplayer(20)) // 设置成圆角图片
    ...
    public void setItems(List<AutoRollLayout.IShowItem> items) {
        this.items = items;
    ...
    public void setAllowAutoRoll(boolean allowAutoRoll) {
        Log.e("setAllowAutoRoll", "开始滚动了：" + allowAutoRoll);
    ...
    public List<AutoRollLayout.IShowItem> getItemList() {
        items = new ArrayList<>();
        return items;
    }
    ...
    private PagerAdapter pagerAdapter = new PagerAdapter() {
        ...
        public Object instantiateItem(ViewGroup container, int position) {
            // 需要构造出ImageView？
            RoundImageViewTo iv = new RoundImageViewTo(container.getContext());
            iv.setType(RoundImageViewTo.TYPE_ROUND);
            iv.setBorderRadius(5);
            iv.setScaleType(ScaleType.FIT_XY);
            /*Picasso.with(mContext).load(items.get(position%items.size()).getImageUrl())
